...
export default function Reports() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            if (fetchError) {
                ...
                const cleaned = ((data as ApplicationRecord[]) ?? []).map((r) => ({
                    ...r,
                    status: r.status != null ? r.status.replace(/^'|'$/g, "") : r.status,
                }));
                setAllRecords(cleaned);
            }
            setLoading(false);
        };
        fetchData();
    ...
    const manilaCount = filtered.filter(
        (r) => r.campus?.toLowerCase().includes("manila")
    ).length;
    const qcCount = filtered.filter(
        (r) => r.campus?.toLowerCase().includes("qc") || r.campus?.toLowerCase().includes("quezon")
    ).length;
...
<div class="report-subtitle">Date Range: ${dateRange} &nbsp;|&nbsp; Generated: ${new Date().toLocaleDateString("en-PH", { year: "numeric", month: "long", day: "numeric" })} &nbsp;|&nbsp; Generated by: ${adminName}</div>
...
<script>window.onload = function() { window.print(); };<\/script>
        ...
        win.document.close();
    }, [filtered, isEmpty, startDate, endDate, session]);
            ...
            {isEmpty ? (
                ...
                <>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                        ...
                        <SummaryCard title="Applicants by Campus (Manila)" value={manilaCount} />
                        <SummaryCard title="Applicants by Campus (QC)" value={qcCount} />
                        <SummaryCard title="Unique Programs Applied" value={byDegree.length} />
                    </div>
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
            ...
            <ResponsiveContainer width="100%" height={220}>
                <BarChart data={data} margin={{ top: 4, right: 8, left: 20, bottom: 40 }}>
                    ...
                    />
                    <Tooltip />
                    <Bar dataKey="count" fill={color} name=" " />
                </BarChart>
            </ResponsiveContainer>
        </div>
    );
