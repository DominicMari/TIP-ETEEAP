...
export default function AdminManagement({ currentUser }: AdminManagementProps) {
  ...
  const handleAddAdmin = async (e: React.FormEvent) => {
    ...
    if (!newAdminName || !newAdminEmail) {
      await showAlert("Please enter both name and email.", "Missing Fields");
      return;
    }
    const emailValidationError = validateEmail(newAdminEmail);
    if (emailValidationError) {
      await showAlert(`Invalid Email: ${emailValidationError}`, "Invalid Email");
      return;
    }
    ...
    try {
      ...
      await showAlert(`Error creating admin: ${(err as Error).message}`, "Error");
  ...
  const saveAdminInfo = async () => {
    ...
    if (currentEmailError) {
      await showAlert("Please fix the email error before saving.", "Validation Error");
      return;
    }

    // Check password match only if a new password was entered
    if (newPassword && newPassword !== confirmPassword) {
      await showAlert("New passwords do not match.", "Password Mismatch");
      return;
    }
    ...
    if (currentPasswordError) {
      await showAlert("Please fix the password errors before saving.", "Validation Error");
      return;
    }
    ...
    try {
      ...
      await showAlert("Admin updated successfully!", "Success", "success");
      ...
      await showAlert(`Error updating admin: ${(err as Error).message}`, "Error", "danger");
    } finally {
      setModalLoading(false); // Stop modal loading indicator
    }
  };
  ...
  const deleteAdmin = async () => {
    ...
    const confirmed = await showConfirm(
      `ARE YOU SURE you want to delete the admin profile for ${selectedAdmin.name} (${selectedAdmin.email})?`,
      "Delete Admin",
      "danger",
      "Yes, Delete",
      "Cancel"
    ...
    try {
      ...
      await showAlert("Admin profile removed from the list successfully.", "Deleted", "success");
      ...
      await showAlert(`Error deleting admin: ${(err as Error).message}`, "Error", "danger");
            ...
            <div className="flex flex-col gap-1 max-h-[60vh] overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100"> {/* Added scrollbar styling */}
              ...
              {admins.map((admin) => (
                <div
                  ...
                  className={`cursor-pointer p-3 rounded-md transition-colors duration-150 ${selectedAdmin?.id === admin.id
