...
export default function InitialForm({
  ...
  const validateAndProceed = () => {
    ...
    if (!folderLink?.trim()) {
      ...
      try { new URL(folderLink); } catch { newErrors.folderLink = "Invalid URL format."; }
    }
  ...
  return (
    ...
    >
      <h2 className="text-center font-bold text-lg sm:text-xl mb-6 text-black">
        APPLICATION FORM AND PRELIMINARY ASSESSMENT FORM
      </h2>

      {/* Name + Photo — photo on top on mobile, side by side on sm+ */}
      <div className="flex flex-col-reverse sm:flex-row gap-4 items-start mb-4">
        ...
        {/* Photo — centered on mobile, right side on sm+ */}
